# BroThatsJS.js - Clean Python Version (MIT License)
# A JavaScript-like interpreter written in Python
...
class Parser:
    ...
    def parse(self):
        ...
        self.skip()
        if self.pos != len(self.text):
            raise SyntaxError("Unexpected character: " + self.text[self.pos])
        return value
...
def interpret_console_log(code, variables):
    ...
    values = [eval_expr(arg, variables) for arg in arguments]
    output = " ".join(Parser("", variables).js_string(v) for v in values)
    print(output)
...
def interpret(code):
    ...
    for stmt in statements:
        interpret_statement(stmt, BroThatsJS_VARIABLES)
...
def show_variables():
    ...
    for name, value in BroThatsJS_VARIABLES.items():
        print(name + " = " + Parser("", BroThatsJS_VARIABLES).js_string(value))
...
print("BroThatsJS interpreter loaded.")
